...
public class CategoryController {
    ...
    @PostMapping("/create")
    public ResponseEntity<CategoryDTO> createCategory(@Valid @RequestBody CategoryDTO dtoObj){
      CategoryDTO createdCategoryDTO =  categoryService.createCategory(dtoObj);
      return new ResponseEntity<>(createdCategoryDTO, HttpStatus.CREATED);
    }

    @PutMapping("/update/{catgId}")
    public ResponseEntity<CategoryDTO> updateCategory(@Valid @RequestBody CategoryDTO dtoObj, @PathVariable("catgId") Integer categoryId){
